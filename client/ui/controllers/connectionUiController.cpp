...
{
    Q_UNUSED(serverId);
    Q_UNUSED(containerIndex);
    Q_UNUSED(clientId);
    // This check existed to protect the currently-connected client profile on a self-hosted-admin
    // (multi-client SSH-deploy) server from being revoked out from under an active connection.
    // SelfHostedAdminServerConfig is gone and no server kind can carry per-client identifiers
    // anymore, so revoking is never blocked here.
    return false;
}
