...
void CoreSignalHandlers::initPrepareConfigHandler()
{
    connect(m_coreController->m_connectionUiController, &ConnectionUiController::prepareConfig, this, [this]() {
        ...
        // TODO: API-subscription config validation removed along with
        // SubscriptionUiController (the api/ subscription business layer); those config
        // kinds no longer have a supported runtime path (see ServersController/
        // ConnectionController), so every remaining default server connects directly.
