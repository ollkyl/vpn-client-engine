...
void ImportController::importConfig(const QJsonObject &config)
{
    ...
    if (credentials.isValid() || config.contains(configKey::containers)) {
        // credentials.isValid() alone matches the old self-hosted-admin SSH-deploy import shape
        // (plain host/port/user/password JSON); that classification no longer exists, so
        // classify first and only proceed if the result is a kind something downstream can
        // actually use. A vpn://-style config with a real `containers` array (VLESS/AmneziaWG
        // etc.) still classifies as ConfigType::Native via hasThirdPartyConfig() and is
        // unaffected by this check.
        const serverConfigUtils::ConfigType configType = serverConfigUtils::configTypeFromJson(config);
        if (configType == serverConfigUtils::ConfigType::Invalid) {
            qDebug() << "Failed to import profile: unrecognized config shape";
            emit importErrorOccurred(ErrorCode::ImportInvalidConfigError, false);
            return;
        }
        m_serversRepository->addServer(QString(), config, configType);
        emit importFinished();
    } else if (config.contains(configKey::configVersion)) {
        // TODO: crc-based duplicate detection removed along with ApiV2ServerConfig
        // (the api/ subscription model layer); gateway/Telegram-sourced API configs no longer
        // have a supported runtime path (see ServersController/ConnectionController), so this
        // branch now just imports without the old dedup check.
