...
ConfigType configTypeFromJson(const QJsonObject &serverConfigObject)
{
    ...
    // Self-hosted-admin/-user classification (SSH-deploy host/port/user/password blobs) was
    // removed along with SelfHostedAdminServerConfig/SelfHostedUserServerConfig; no remaining
    // code path can act on either kind, so a config that isn't API-sourced and isn't a
    // third-party (Native) config is simply not a supported shape.
